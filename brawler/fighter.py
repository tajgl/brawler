...
class Fighter():
    ...
    def move(self, SCREEN_WIDTH, SCREEN_HEIGHT, surface, target, round_over):
        ...
        #can only preform other action if not attacking
        ...
        if self.rect.left + dx < 0:   #left border
            dx = -self.rect.left
        if self.rect.right + dx > 1500:  #right border
            dx = 1500 - self.rect.right 
        if self.rect.bottom + dy > SCREEN_HEIGHT - 110:  #bottom border
            self.vel_y = 0
            self.jump = False
    ...
    def update_action(self, new_action):
        #check if the new action is different than the previous
